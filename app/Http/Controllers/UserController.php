...
class UserController extends Controller
{
        ...
        if (\Auth::user()->can('manage user')) {
            ...
            if ($user->type == 'super admin') { 
                if (!empty($filter)) {
                    if($filter==='t'){
                        // DB::enableQueryLog();
                        $users = User::Today()->where('type', '!=', 'super admin')->where('created_by', '=', $user->creatorId())->orderBy('created_at', 'desc')->paginate(10);
                        // $query = DB::getQueryLog();
                        // dd($query); 
                    } 
                    if($filter==='y'){
                        $users = User::Yesterday()->where('type', '!=', 'super admin')->where('created_by', '=', $user->creatorId())->orderBy('created_at', 'desc')->paginate(10);
                    } 
                    if($filter==='7d'){
                        $users = User::Last7Days()->where('type', '!=', 'super admin')->where('created_by', '=', $user->creatorId())->orderBy('created_at', 'desc')->paginate(10);
                    } 
                    if($filter==='30d'){
                        $users = User::Last30Days()->where('type', '!=', 'super admin')->where('created_by', '=', $user->creatorId())->orderBy('created_at', 'desc')->paginate(10);
                    } 
                }else{
                    $users = User::LastYear()->where('type', '!=', 'super admin')->where('created_by', '=', $user->creatorId())->orderBy('created_at', 'desc')->paginate(10);  
                }
            } else {  
                if (!empty($filter)) {
                    if($filter==='t'){
                        $users = User::Today()->where('created_by', '=', $user->creatorId())->orderBy('created_at', 'desc')->paginate(10);  
                    } 
                    if($filter==='y'){
                        $users = User::Yesterday()->where('created_by', '=', $user->creatorId())->orderBy('created_at', 'desc')->paginate(10);  
                    } 
                    if($filter==='7d'){
                        $users = User::Last7Days()->where('created_by', '=', $user->creatorId())->orderBy('created_at', 'desc')->paginate(10);  
                    } 
                    if($filter==='30d'){
                        $users = User::Last30Days()->where('created_by', '=', $user->creatorId())->orderBy('created_at', 'desc')->paginate(10);  
                    } 
                }else{
                    $users = User::where('created_by', '=', $user->creatorId())->orderBy('created_at', 'desc')->paginate(10);  
                }
            }  
            return view('user.index')->with('users', $users)->with('filter', $filter); 
        } else {
    ...
    public function editprofile(Request $request)
    {
        ...
        return redirect()->back()->with(
            'success', 'Profile successfully updated.' . ((isset($result) && $result != 1) ? '<br> <span class="text-danger">' . $result . '</span>' : '')
        );
    }
