            ...
            <ul class="dash-navbar">
                ...
                @if(\Auth::user()->can('manage business'))
                    @if(\Auth::user()->can('create business'))
                        ...
                        @if($bussiness_id==''|| is_null($bussiness_id))
                        <li class="dash-item {{ (Request::segment(1) == 'business')?'active':''}}">
                            <a href="#" class="dash-link" data-bs-toggle="modal" data-bs-target="#exampleModal" data-url="{{ route('business.create') }}" data-size="xl" data-bs-whatever="{{ __('Create New Business') }}"> <span class="dash-micon"><i class="ti ti-credit-card"></i></span><span class="dash-mtext">{{ __('Create business') }}</span>
                            </a>
                        </li>
                        @else
                        <li class="dash-item {{ (Request::segment(1) == 'business')?'active':''}}">
